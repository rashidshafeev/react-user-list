...
function* workGetUserFetch(): any {
    ...
    try {
        ...
        users.data.forEach( (user : BlogUserExternal) => {
            if ((usersStore.find( (u : BlogUser) => u.username.toLowerCase() == user.username.toLowerCase()))||(usersStore.find( (u : BlogUser) => u.email.toLowerCase() == user.email.toLowerCase())))  {
              errorMessage = "Некоторые пользователи не были добавлены, из-за совпадения юзернейма или почты; "
            } else {
              usersAdd.push(user)
            }
